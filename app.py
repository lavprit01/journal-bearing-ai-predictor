...
st.set_page_config(page_title="Journal Bearing AI", page_icon="⚙️", layout="wide")
...
st.markdown("**AI-powered prediction using Artificial Neural Networks** *Based on: Applied Tribology — Khonsari & Booser (Table 8.6)* *Project by: Lavprit Anand | MNNIT Allahabad Summer-Internship 2026*")
st.divider()

app_mode = st.sidebar.radio("Select Application Mode:", ["1. Direct Predictor (Given ε)", "2. Thermodynamic Solver (Iterative)"])
st.sidebar.divider()
...
if "1." in app_mode:
    ...
    if 'last_LD' not in st.session_state: st.session_state.last_LD = None
    if 'last_eps' not in st.session_state: st.session_state.last_eps = None

    with st.sidebar:
        st.header("📥 Input Parameters")
        LD_val = st.slider("Aspect Ratio L/D", 0.125, 2.0, 1.0, 0.001)
        eps_val = st.slider("Eccentricity Ratio ε", 0.05, 0.95, 0.50, 0.01)
        model_choice = st.multiselect("Select Models", ['FFNN', 'RBNN', 'GRNN'], default=['FFNN', 'RBNN', 'GRNN'])
        st.divider()
    ...
    if st.session_state.predictions is not None:
        ...
        LD_used, eps_used = st.session_state.last_LD, st.session_state.last_eps
        ...
        for col, icon, name, key, unit in zip([c1,c2,c3,c4,c5,c6,c7,c8], icons, param_names, param_keys, units):
            col.metric(label=f"{icon} {key}", value=f"{model_preds['FFNN'][param_keys.index(key)]:.4f} {unit}", help=name)
    ...
    if solve_btn:
        with st.spinner("AI is solving the thermal loop..."):
            ...
            for i in range(1, 21):
                ...
                try: eps_found = brentq(obj_func, 0.05, 0.95, args=(S_target,))
                except ValueError: 
